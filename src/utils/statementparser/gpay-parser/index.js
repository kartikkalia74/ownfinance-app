...
// ₹AMOUNT
const GPAY_TRANSACTION_REGEX = /(\d{2}\s+\w{3},\s+\d{4})\s+(\d{1,2}:\d{2}\s+(?:AM|PM))\s+(Paid to|Received from)\s+([^\n]+?)\s+UPI Transaction ID:\s+(\d+)\s+(Paid (?:by|to)\s+[^\n]+?)\s+₹([\d,]+)/gs;
...
function extractTransactions(text) {
    ...
    for (const match of matches) {
        ...
        const type = match[3]; // "Paid to" or "Received from"
        const name = match[4].trim(); // "Akhil Sharma"
        const upiTransactionId = match[5]; // "114999892784"
        let bankInfo = match[6]; // "Paid by HDFC Bank 4230"
        const amount = match[7]; // "200" or "19,000"

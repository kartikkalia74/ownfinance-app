...
export function cleanHDFCStatementText(text) {
    ...
    cleaned = cleaned.replace(/^(?:Txn\s+Date|Date\s+Narration|Page\s+\d+|Page\s+No).*/gmi, '');
    ...
    // Remove numerical summary line (Opening Bal, Dr Count, Cr Count, Debits, Credits, Closing Bal)
    // Pattern: Amount Int Int Amount Amount Amount
        ...
        if (dateMatch) {
            ...
            for (let j = 0; j < txnLines.length; j++) {
                ...
                if (amountsMatch) {
                    ...
                    console.log(amounts, amountsMatch, "amount")
                    if (amounts.length === 3) {

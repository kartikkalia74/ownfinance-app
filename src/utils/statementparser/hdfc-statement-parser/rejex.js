...
/**
 * Cleans HDFC statement text by removing headers, footers, and other metadata
 * @param {string} text - The raw statement text
 * @returns {string} Cleaned text ready for transaction extraction
 */
export function cleanHDFCStatementText(text) {
    if (!text) return '';

    let cleaned = text;

    // 1. Remove table headers and page markers (line-based, no lookaheads)
    // Improved to handle leading whitespace and different page formats
    cleaned = cleaned.replace(/^\s*(?:Txn\s+Date|Date\s+Narration|Page\s+\d+|Page\s+No).*/gmi, '');

    // 1.1 Remove recurring page headers/footers interlaced in text
    cleaned = cleaned.replace(/.*Hdfc\s+Bank\s+Ltd.*/gmi, '');
    cleaned = cleaned.replace(/.*SAVINGS\s+-\s+RESIDENTS.*/gmi, '');
    cleaned = cleaned.replace(/.*Debit\s+Count\s+Credit\s+Count.*/gmi, '');
    cleaned = cleaned.replace(/.*Details\s+of\s+TD\s+Interest.*/gmi, '');
    cleaned = cleaned.replace(/.*Nominee\s+Details\s+for\s+FDs.*/gmi, '');
    cleaned = cleaned.replace(/^\s*[\d,.]+\s+\d+\s+\d+\s+[\d,.]+\s+[\d,.]+\s+[\d,.]+\s*$/gm, '');

    // 1.2 Aggressive removal of user details block often found at top of pages
    // Pattern often includes Name, Customer ID, Branch, etc.
    cleaned = cleaned.replace(/^\s*Kartik\s+Kalia.*$/gmi, '');

    // Remove lines that start with a colon (continuation of metadata)
    cleaned = cleaned.replace(/^\s*:\s*.*$/gm, '');

    // Remove specific keywords often found in the header address block
    // Added \s* to handle indentation
    cleaned = cleaned.replace(/^\s*(?:Phase\s+Vii|Scf\s+\d+|Mohali|Punjab|Chandigarh|Panchkula|Hdfc\s+Bank\s+Ltd).*/gmi, '');
    cleaned = cleaned.replace(/^\s*\d{6}\s*$/gm, ''); // Remove 6-digit pin codes on their own line

    // Remove "SUMMARY" header and small integer counts (Debit Count/Credit Count lines)
    cleaned = cleaned.replace(/^\s*SUMMARY\s*$/gmi, '');
    cleaned = cleaned.replace(/^\s*\d+\s+\d+\s*$/gm, ''); // Matches "22 12" type lines
    // Remove summary line with 4 amounts (Opening, Debit, Credit, Closing)
    cleaned = cleaned.replace(/^\s*[\d,.]+\s+[\d,.]+\s+[\d,.]+\s+[\d,.]+\s*$/gm, '');

    // 2. Remove known metadata labels at start of lines (single line only to be safe)
    const keywords = [
        'Customer ID', 'Cust ID', 'Account Name', 'Customer Name', 'Account Number', 'Account No',
        'Account Branch', 'Account Type', 'Statement From', 'Joint Holders', 'Nomination',
        'RTGS/NEFT IFSC', 'A/C Open Date', 'Account Status', 'Branch Code',
        'Preferred Customer', 'Address', 'City', 'State', 'Phone no.', 'Email',
        'POST OFFICE', 'S/O', 'M/S.', 'VILLAGE', 'TEHSIL', 'P.O.', 'Currency',
        'Statement of account', 'Branch', 'Saving Account Details', 'Savings Account Details',
        'Opening Balance', 'Closing Balance', 'Generated By', 'Requesting Branch Code',
        'Tax', 'Interest Paid', 'Deducted', 'Compounded', 'FD CCY', 'FD Number', '\\(In INR\\)'
    ];

    // Construct a single regex for efficiency, anchored to start of line
    const metadataRegex = new RegExp(`^\\s*(?:${keywords.map(kw => kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')}).*`, 'gmi');
    cleaned = cleaned.replace(metadataRegex, '');

    // 3. Remove Footer and Summary sections (from marker to end)
    const footerMarkers = [
        'STATEMENT SUMMARY', 'Generated On', 'This is a computer generated',
        'HDFC BANK LIMITED', '*Closing balance includes', 'Contents of this statement',
        'Registered Office Address', 'State account branch GSTN', 'HDFC Bank GSTIN',
        'Total Withdrawal Balance', 'Total Sweep In FD Linked', 'FD may be linked to other Accounts'
    ];
    for (const marker of footerMarkers) {
        const escapedMarker = marker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const markerRegex = new RegExp(`(?:\\n|^)\\s*${escapedMarker}[\\s\\S]*$`, 'i');
        if (markerRegex.test(cleaned)) {
...
export function extractTransactionsAdvanced(text) {
    ...
    // Use modular cleaning function
    const cleanText = cleanHDFCStatementText(text);
    ...
    while (i < lines.length) {
        ...
        if (dateMatch) {
            ...
            while (i < lines.length) {
                ...
                // Stop at date or summary/footer
                if (nextLine.match(/^\d{2}\/\d{2}\/\d{2,4}\s+/) ||
                    nextLine.match(/STATEMENT\s+SUMMARY|Generated\s+On|This\s+is\s+a\s+computer|HDFC\s+BANK\s+(?:LIMITED|LTD)/i) ||
                    nextLine.match(/SUMMARY\s+[\d,.]+\s+Debit\s+Count/i)) {
                    break;
                }

                // Explicitly stop at Page headers or strong metadata start triggers if they slipped through cleaning
                // This prevents "Page X of Y" and "Customer ID: ..." from being appended to narration
                if (nextLine.match(/^\s*Page\s+\d+\s+of\s+\d+/i) ||
                    nextLine.match(/^\s*Customer\s+ID\s*:/i) ||
                    nextLine.match(/^\s*Statement\s+From\s*:/i)) {
                    // We found a header block that wasn't cleaned. 
                    // We should likely skip this line and subsequent header lines until we find something that looks like transaction part or next date
                    // For now, just breaking will leave it for the main loop to handle (or skip if it's garbage)
                    break;
                }

                // Skip headers (either the classic vertical header or the horizontal table header)
                // Expanded to be more aggressive against table headers
                const isClassicHeader = nextLine.match(/Txn\s+Date|Narration|Withdrawals|Deposits|Closing\s+Balance|Opening\s+Balance|Limit/i);
                const isHorizontalHeader = nextLine.match(/Date\s+Narration\s+Chq\.\/Ref\.No\./i) || nextLine.match(/Value\s+Dt\s+Withdrawal\s+Amt/i);

                if (isClassicHeader || isHorizontalHeader) {
                    i++;
                    continue;
                }
            ...
            for (let j = 0; j < txnLines.length; j++) {
                ...
                if (amountsMatch) {
                    const amounts = amountsMatch[1].trim().split(/\s+/)
                    // .filter(a => a);
